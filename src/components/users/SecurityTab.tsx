...
{
  id: 'confirmPw', label: 'Confirm new password', autoComplete: 'new-password',
          ...
          <div key={field.id} className={'input-wrapper'}>
            ...
            {validationErr.errField === field.id &&
              <span id={`${field.id}-err`} className="alert-text label-required"
                aria-describedby="currentPw-error" style={{fontSize:'.685rem', fontWeight:'500'}}>{validationErr.err}</span>}
          </div>
        )}
      </div>
      <button
        ...
        className="submit-btn el-flx" style={{alignItems: 'center' , justifyContent:'center', gap:'.5em'}}>
        <IconWrapper name="LuLock"/>
          Change Password
        </button>
    </fetcher.Form >)
